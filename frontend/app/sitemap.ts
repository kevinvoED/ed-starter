...
import { urlQuery } from "@/sanity/queries/sharedFragments";
...
const SITEMAP_QUERY = groq`
  *[_type in $viewableTypes && !meta.noindex && defined(slug)] {
    ...
    // look for hero-like modules and grab their first image as the images for this item
    // in order to append the width limit, we need to unpack the image urls
    "images": select(
      modules[0]._type match "hero*" && defined(modules[0].image) => modules[0]{
        "images": array::compact([image.asset->{ "url": url + "?w=800&amp;fit=max" }.url, null])
      }.images, null),
    // TODO: if there are videos in modules, we can add those here with a similar query
    "videos": null
