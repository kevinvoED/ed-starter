...
import type { RESOURCE_CATEGORY_COUNT_QUERYResult, Slug } from "@/sanity.types";
...
export const PostCategoryItem = ({
      ...
      return (
        ...
        >
          ...
          <span className="relative z-10 translate-x-[20px]">
            {title} [{categoriesCount.totalPostCount ?? 0}]
          </span>
        </button>
      );
    }
  ...
  if (
    category &&
    slug?.current &&
    kebabCase(category) === kebabCase(slug.current)
  ) {
    return (
      <button
        type="button"
      ...
      >
        <Dot
          color="neon"
          className="absolute top-1.5 left-1 translate-x-1" // translate-x-1 = 4px to match hover end state
        />
        <span className="relative z-10 translate-x-[20px]">
          {" "}
          {title} [
          ...
          !category ||
          kebabCase(category) === kebabCase(slug?.current ?? "")
            ? categoriesCount.categories?.find((c) => c._id === _id)?.count
            : 0}
          ]
        </span>
      </button>
